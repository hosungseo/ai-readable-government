...
이 저장소는 정부 보도자료, 관보, 국무회의 회의록 같은 원천 문서를 readable corpus로 묶고, 동시에 `robots.txt`, `sitemap.xml`, `llms.txt`, `agent-catalog.json` 같은 agent-facing surface를 제공합니다. 즉, 단순 프록시가 아니라 **(1) readable corpus 그룹, (2) transition-request / transition-intelligence layer, (3) government API bundle**을 함께 읽게 하는 프로젝트입니다.
...
- **2축, transition-request / transition-intelligence 레이어**
  - `public-data-portal-intelligence` = 파일데이터→API 전환 후보를 구조화하는 검토/제안 레이어
- **3축, government API bundle**
  - 공공데이터포털, ECOS, 국회, 법제처, KOSIS 등 입력된 공공 API를 일괄적으로 제공하는 축
- `ai-readable-government` = 이 3축을 함께 읽게 하는 public reader and discovery layer

## 3가지 방향

### 1. Readable corpus 그룹
관보, 보도자료, 국무회의 회의록 같은 공개 문서를 사람이 읽기 쉽고 AI가 처리하기 쉬운 corpus로 다시 묶는 축입니다. sample reader, metadata, source link, full corpus discovery가 이 축에 속합니다.
...
### 2. Transition-request / transition-intelligence 레이어
파일데이터를 이 저장소가 직접 authoritative API로 바꾸는 것이 아니라, 어떤 데이터가 API 전환 대상으로 구조화되어야 하는지 드러내는 축입니다. 즉, 전환 실행기라기보다 전환 필요성과 우선순위를 드러내는 요청 레이어입니다.
...
### 3. Government API bundle
이미 존재하는 정부 API를 한곳에서 발견하고 읽을 수 있게 정리하는 축입니다. 이 저장소가 모든 데이터를 직접 제공하기보다, 어디에 무엇이 있고 어떻게 접근해야 하는지 보여 주는 catalog 성격을 가집니다.

현재 이 축에는 공공데이터포털, ECOS, 국회, 법제처, KOSIS 등 제가 입력받은 공공 API 묶음이 포함됩니다.
...
- **3축, government API bundle**
  - 공공데이터포털, ECOS, 국회, 법제처, KOSIS 등 입력된 공공 API 묶음
